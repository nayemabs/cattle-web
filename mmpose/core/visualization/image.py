...
def imshow_bboxes(img,
    ...
    img = mmcv.imshow_bboxes(
        ...
        out_file=None)
    data = []
    with open('/home/abs/mmpose/bbox_rear.csv', 'a') as f:
        writer = csv.writer(f)
...
def imshow_keypoints(img,
    ...
    kptID=x0=y0=x1=y1=x2=y2=x3=y3=0

    # header = ['0x','0y','1x','1y','5x','5y','2x','2y','4x','4y','3x','3y']
    data = []
    with open('/home/abs/mmpose/kpt_rear.csv', 'a') as f:
        writer = csv.writer(f)
        ...
        width = round(((y1-y0)**2+(x1-x0)**2)**0.5)
        height = round(((y3-y2)**2+(x3-x2)**2)**0.5)
